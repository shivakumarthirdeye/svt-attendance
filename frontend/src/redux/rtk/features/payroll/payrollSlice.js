...
export const addPayslipAdvance = createAsyncThunk(
  'payslip/addPayslipAdvance',
  async values => {
    try {
      const { data } = await axios.post(`payroll/add-advance`, values);
      return data;
    } catch (error) {
      console.log(error.message);
    }
...
export const getPayslipAdvanceHistory = createAsyncThunk(
  'payslip/addPayslipAdvance',
  async values => {
    try {
      const { data } = await axios.get(`payroll/advance-history`, values);
      return data;
    } catch (error) {
      console.log(error.message);
    }
